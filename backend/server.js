...
// make sure that when we configure a port and receivr it via an environment variable, it's a valid number

const normalizePort = (originalPort) => {
    //parcing our port and it returns an integer
    const port = parseInt(originalPort, 10);

    // if the port number is illegal, it returns our original port
    if(isNaN(port)) {return originalPort;}
...
switch (error.code) {
    ...
    case "EADDRINUSE":
        console.error(bind + " is already in use") // the address is already in use
        process.exit(1);
        default: throw error;
}
};
